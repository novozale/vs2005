Public Class ItemSelectList2
    ...
    Private Sub ItemSelectList2_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        MySQLStr = MySQLStr & "(tbl_ForecastOrderR3_Main.Code IN "
        ...
        If Trim(MainForm.TextBox4.Text) = "" Then
            ...
            MySQLStr = MySQLStr & "AND ((UPPER(tbl_ForecastOrderR3_Main.Code) LIKE N'%" & UCase(MainForm.TextBox1.Text) & "%') "
            MySQLStr = MySQLStr & " OR (UPPER(tbl_ForecastOrderR3_Main.Name) LIKE N'%" & UCase(MainForm.TextBox1.Text) & "%')) "
        Else
            If Trim(MainForm.TextBox1.Text) = "" Then
                '----Во второе окно условие не введено
                MySQLStr = MySQLStr & "AND ((UPPER(tbl_ForecastOrderR3_Main.Code) LIKE N'%" & UCase(MainForm.TextBox4.Text) & "%') "
                MySQLStr = MySQLStr & " OR (UPPER(tbl_ForecastOrderR3_Main.Name) LIKE N'%" & UCase(MainForm.TextBox4.Text) & "%')) "
            Else
                '----Условия введены в оба окна
                MySQLStr = MySQLStr & "AND (((UPPER(tbl_ForecastOrderR3_Main.Code) LIKE N'%" & UCase(MainForm.TextBox4.Text) & "%') "
                MySQLStr = MySQLStr & "AND (UPPER(tbl_ForecastOrderR3_Main.Code) LIKE N'%" & UCase(MainForm.TextBox1.Text) & "%')) "
                MySQLStr = MySQLStr & " OR ((UPPER(tbl_ForecastOrderR3_Main.Name) LIKE N'%" & UCase(MainForm.TextBox4.Text) & "%') "
                MySQLStr = MySQLStr & " AND (UPPER(tbl_ForecastOrderR3_Main.Name) LIKE N'%" & UCase(MainForm.TextBox1.Text) & "%'))) "
            End If
        End If
