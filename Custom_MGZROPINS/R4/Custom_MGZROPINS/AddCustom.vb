Public Class AddCustom
    ...
    Private Sub AddCustom_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        '////////////////////////////////////////////////////////////////////////////////
        '//
        ...
        Label3.Text = "Установка ручных значений МЖЗ, ROP и страхового уровня для запаса " & Trim(MainForm.DataGridView1.SelectedRows.Item(0).Cells(0).Value.ToString()) & " " & Trim(MainForm.DataGridView1.SelectedRows.Item(0).Cells(1).Value.ToString())
        TextBox6.Text = Trim(MainForm.DataGridView1.SelectedRows.Item(0).Cells(7).Value.ToString())
        TextBox4.Text = Trim(MainForm.DataGridView1.SelectedRows.Item(0).Cells(8).Value.ToString())
        TextBox3.Text = Trim(MainForm.DataGridView1.SelectedRows.Item(0).Cells(9).Value.ToString())
    ...
    Private Function CheckDataFiling() As Boolean
        ...
        Dim MyMonthNum As Double                 '--кол - во месяцев, за которое делается расчет
        ...
        If DateTimePicker1.Value < Now() Then
            MsgBox("Дата ""Действует до даты:"" должна быть больше текущей", MsgBoxStyle.Critical, "Внимание!")
        ...
        If DateTimePicker1.Value > DateAdd(DateInterval.Month, MyMonthNum, Now()) Then
            MsgBox("Дата ""Действует до даты:"" не должна превышать текущую больше чем на " & CStr(MyMonthNum) & " месяцев.", MsgBoxStyle.Critical, "Внимание!")
            DateTimePicker1.Select()
            CheckDataFiling = False
            Exit Function
        End If
