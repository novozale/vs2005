Public Class SupplierSelect
    ...
    Private Sub SupplierSelect()
        ...
        If MySrcWin = "ADD" Then
            MySQLStr = "INSERT INTO tbl_SupplSearch_Suppliers "
            MySQLStr = MySQLStr & "(SupplSearchID, SupplierCode, SupplierName) "
            MySQLStr = MySQLStr & "VALUES (" & CStr(Declarations.MyRequestNum) & ", "
            MySQLStr = MySQLStr & "N'" & Trim(DataGridView1.SelectedRows.Item(0).Cells(0).Value) & "', "
            MySQLStr = MySQLStr & "N'" & Trim(DataGridView1.SelectedRows.Item(0).Cells(1).Value) & "') "
            InitMyConn(False)
            Declarations.MyConn.Execute(MySQLStr)
        ElseIf MySrcWin = "UPDATE" Then
            MySQLStr = "UPDATE tbl_SupplSearch_Suppliers "
            MySQLStr = MySQLStr & "SET SupplierCode = N'" & Trim(DataGridView1.SelectedRows.Item(0).Cells(0).Value) & "', "
            MySQLStr = MySQLStr & "SupplierName = N'" & Trim(DataGridView1.SelectedRows.Item(0).Cells(1).Value) & "' "
            MySQLStr = MySQLStr & "WHERE (ID = " & Trim(MainForm.DataGridView3.SelectedRows.Item(0).Cells(0).Value) & ") "
            InitMyConn(False)
            Declarations.MyConn.Execute(MySQLStr)
        ElseIf MySrcWin = "SELECT" Then
            MyItemSelect.TextBox1.Text = Trim(Me.DataGridView1.SelectedRows.Item(0).Cells(0).Value.ToString())
            If MyItemSelect.TextBox1Validating() = True Then
                MyItemSelect.TextBox1Validated()
            End If
        End If
        Me.Close()
    End Sub
