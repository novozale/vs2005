...
MyCatch:
    ...
    Public Function CheckNegativeMargin(ByVal OrderNum As String, ByVal MarginType As Integer) As Boolean
        ...
        On Error GoTo MyCatch
        'MyConn1 = New ADODB.Connection
        'MyConn1.CursorLocation = 3
        'MyConn1.CommandTimeout = 600
        'MyConn1.ConnectionTimeout = 300
        'MyConn1.Open(MyScala.ActiveProcess.UserContext.GetConnectionString(1))
        ...
        Else
            ...
            If My.Settings.UseOffice = "LibreOffice" Then
                ...
                '-----Ширина колонок
                oSheet.getColumns().getByName("A").Width = 3000
                oSheet.getColumns().getByName("B").Width = 3000
                ...
                LOFontSetSize(oServiceManager, oDispatcher, oFrame, "A7:D7", 11)
                oSheet.getCellRangeByName("A7:D7").CellBackColor = 16775598
                ...
                While Not Declarations.MyRec.EOF
                    oSheet.getCellRangeByName("A" & i).String = Declarations.MyRec.Fields("Str").Value
                    oSheet.getCellRangeByName("B" & i).String = Declarations.MyRec.Fields("Code").Value
                    oSheet.getCellRangeByName("C" & i).String = Declarations.MyRec.Fields("Name").Value
                    oSheet.getCellRangeByName("D" & i).Value = Declarations.MyRec.Fields("COST").Value

                    Declarations.MyRec.MoveNext()
                    i = i + 1
                End While
                LOFontSetFamilyName(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":D" & i, "Calibri")
                LOFontSetSize(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":D" & i, 10)
                LOWrapText(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":D" & i)
                LOFormatCells(oServiceManager, oDispatcher, oFrame, "D" & Counter & ":D" & i, 4)
            ...
            Else
                ...
                'MyWRKBook.ActiveSheet.Range("E7") = "Себестоимость"
                'MyWRKBook.ActiveSheet.Range("F7") = "Маржа"
                ...
                While Not Declarations.MyRec.EOF
                    ...
                    'MyWRKBook.ActiveSheet.Range("F" + CStr(i)) = IIf(Declarations.MyRec.Fields("COST").Value = 0, 0, IIf(Declarations.MyRec.Fields("PriCOST").Value = 0, 0, System.Math.Round((Declarations.MyRec.Fields("COST").Value - Declarations.MyRec.Fields("PriCOST").Value) / Declarations.MyRec.Fields("COST").Value * 100, 2)))
                    Declarations.MyRec.MoveNext()
                    i = i + 1
                End While
...
MyCatch:
    ...
    Public Function CheckNegativeProjectMargin(ByVal OrderNum As String) As Boolean
        ...
        If Declarations.MyRec.EOF = True And Declarations.MyRec.BOF = True Then
            CheckNegativeProjectMargin = True
        Else
            '---Есть отрицательная маржа, ее и выводим
            If My.Settings.UseOffice = "LibreOffice" Then
                Dim oServiceManager As Object
                ...
                '---заголовок общий
                oSheet.getCellRangeByName("B1").String = "Заказ на продажу номер " & OrderNum
                oSheet.getCellRangeByName("B2").String = "В этом заказе существуют запасы с маржой ниже разрешенной "

                LOFontSetFamilyName(oServiceManager, oDispatcher, oFrame, "A1:B2", "Calibri")
                LOFontSetSize(oServiceManager, oDispatcher, oFrame, "A1:B2", 11)
                LOFontSetBold(oServiceManager, oDispatcher, oFrame, "B1")
                ...
                LOFontSetSize(oServiceManager, oDispatcher, oFrame, "A5:F5", 11)
                oSheet.getCellRangeByName("A5:F5").CellBackColor = 16775598
                Dim LineFormat As Object
                LineFormat = oServiceManager.Bridge_GetStruct("com.sun.star.table.BorderLine2")
                LineFormat.LineStyle = 0
                LineFormat.LineWidth = 70
                ...
                While Not Declarations.MyRec.EOF
                    ...
                    oSheet.getCellRangeByName("C" & i).String = Declarations.MyRec.Fields("Name").Value
                    oSheet.getCellRangeByName("D" & i).Value = Declarations.MyRec.Fields("COST").Value
                    oSheet.getCellRangeByName("E" & i).Value = Declarations.MyRec.Fields("PriCOST").Value
                    oSheet.getCellRangeByName("F" & i).Value = IIf(Declarations.MyRec.Fields("COST").Value = 0, _
                        0, IIf(Declarations.MyRec.Fields("PriCOST").Value = 0, 0, Math.Round((Declarations.MyRec.Fields("COST").Value - _
                        Declarations.MyRec.Fields("PriCOST").Value) / Declarations.MyRec.Fields("COST").Value * 100, 2)))

                    Declarations.MyRec.MoveNext()
                    i = i + 1
                End While
                LOFontSetFamilyName(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":F" & i, "Calibri")
                LOFontSetSize(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":F" & i, 10)
                LOWrapText(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":F" & i)
                LOFormatCells(oServiceManager, oDispatcher, oFrame, "D" & Counter & ":F" & i, 4)
            ...
            Else
                ...
                MyObj.Application.Visible = True
                MyObj = Nothing
                CheckNegativeProjectMargin = False
            End If
        End If
        'MyRec1.Close()
        'MyRec1 = Nothing
...
MyCatch:
    ...
    Public Function CheckCreditLimit(ByVal OrderNumber As String, ByVal CustomerID As String) As Boolean
        ...
        MySQLStr = MySQLStr & "WHERE (SL03004 <= GETDATE()) AND "
...
MyCatch:
    ...
    Public Function GetShippingList(ByVal MyOrderID As String)
        '////////////////////////////////////////////////////////////////////////////////////////
        '// Получение списка подборки  с сервера отчетов
        '// 
        '////////////////////////////////////////////////////////////////////////////////////////
        ...
        rs.Url = My.Settings.ES_ShippingCheck_2_spbprd5_ReportExecutionService
        ...
        Dim parameters(2) As spbprd5.ParameterValue
        parameters(0) = New spbprd5.ParameterValue()
        parameters(0).Name = "OrderNumber"
        parameters(0).Value = MyOrderID
        parameters(1) = New spbprd5.ParameterValue()
        parameters(1).Name = "Sort"
        parameters(1).Value = 3
        parameters(2) = New spbprd5.ParameterValue()
        parameters(2).Name = "PickList"
        ...
        Try
            ...
            FName = IO.Path.GetTempPath + "ShippingList_" + MyOrderID + "_" + CStr(DatePart(DateInterval.Year, Now())) + "-" + CStr(DatePart(DateInterval.Month, Now())) + "-" + CStr(DatePart(DateInterval.Day, Now)) + ".xls"
            Using stream As IO.FileStream = IO.File.OpenWrite(FName)
            ...
            If My.Settings.UseOffice = "LibreOffice" Then
                ...
                oServiceManager = CreateObject("com.sun.star.ServiceManager")
                oDesktop = oServiceManager.createInstance("com.sun.star.frame.Desktop")
                oFileName = Replace(FName, "\", "/")
                oFileName = "file:///" + oFileName
                Dim arg(1)
                arg(0) = mAkePropertyValue("Hidden", False, oServiceManager)
                arg(1) = mAkePropertyValue("MacroExecutionMode", 4, oServiceManager)
                oWorkBook = oDesktop.loadComponentFromURL(oFileName, "_blank", 0, arg)
            Else
                MyObj = CreateObject("Excel.Application")
                MyObj.Workbooks.Open(FName)
                MyObj.Application.Visible = True
            End If
    ...
    Public Function CheckSalesPrices(ByVal OrderNum As String) As Boolean
        ...
        Else
            If My.Settings.UseOffice = "LibreOffice" Then
                ...
                '-----Ширина колонок
                oSheet.getColumns().getByName("A").Width = 7000
                oSheet.getColumns().getByName("B").Width = 7000
                oSheet.getColumns().getByName("C").Width = 7000

                '---заголовок общий
                oSheet.getCellRangeByName("B1").String = "Заказ на продажу номер " & OrderNum
                oSheet.getCellRangeByName("B2").String = "В этом заказе после перевода в 1 тип изменилась цена: "
                ...
                LOFontSetSize(oServiceManager, oDispatcher, oFrame, "A5:C5", 11)
                oSheet.getCellRangeByName("A5:C5").CellBackColor = 16775598
                Dim LineFormat As Object
                LineFormat = oServiceManager.Bridge_GetStruct("com.sun.star.table.BorderLine2")
                LineFormat.LineStyle = 0
                LineFormat.LineWidth = 70
                ...
                'Declarations.MinMargin = Declarations.MyRec.Fields("MinMargin").Value
                While Not Declarations.MyRec.EOF
                    oSheet.getCellRangeByName("A" & i).String = Declarations.MyRec.Fields("StockCode").Value
                    oSheet.getCellRangeByName("B" & i).String = Declarations.MyRec.Fields("Price").Value
                    oSheet.getCellRangeByName("C" & i).String = Declarations.MyRec.Fields("ConvertPrice").Value

                    Declarations.MyRec.MoveNext()
                    i = i + 1
                End While
                LOFontSetFamilyName(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":C" & i, "Calibri")
                LOFontSetSize(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":C" & i, 10)
                LOWrapText(oServiceManager, oDispatcher, oFrame, "A" & Counter & ":C" & i)
                LOFormatCells(oServiceManager, oDispatcher, oFrame, "B" & Counter & ":C" & i, 4)
